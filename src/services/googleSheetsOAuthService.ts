...
export class GoogleSheetsOAuthService {
  ...
  private async createDefaultTeacher() {
    ...
    if (googleOAuthService.isAuthenticated()) {
      try {
        ...
        console.error('Failed to create default teacher in sheets:', error);
      }
    }
  ...
  private async makeAuthenticatedRequest(range: string, method: 'GET' | 'PUT' | 'POST' | 'DELETE' = 'GET', data?: any) {
    if (!SPREADSHEET_ID) {
      throw new Error('Google SpreadsheetIDが設定されていません');
    }

    const accessToken = await googleOAuthService.getValidAccessToken();
    if (!accessToken) {
      throw new Error('Google OAuth2認証が必要です');
    }

    let url = `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${range}`;
    
    if (method === 'POST') {
      url = `${url}:append?valueInputOption=RAW`;
    } else if (method === 'PUT') {
      url = `${url}?valueInputOption=RAW`;
    }
  ...
  private async loadUsersFromSheets(): Promise<void> {
    try {
      const response = await this.makeAuthenticatedRequest('Users!A:F');
  ...
  private async loadTodosFromSheets(): Promise<void> {
    try {
      const response = await this.makeAuthenticatedRequest('Todos!A:F');
      const rows = response.values || [];
  ...
  private async addUserToSheets(user: User): Promise<void> {
    await this.makeAuthenticatedRequest('Users', 'POST', {
      values: [[
        user.id,
        user.username,
        user.role,
  ...
  private async addTodoToSheets(todo: TodoRecord): Promise<void> {
    await this.makeAuthenticatedRequest('Todos', 'POST', {
      values: [[
        todo.id,
        todo.user_id,
        todo.text,
  ...
  // Public methods
  public isOAuthAuthenticated(): boolean {
  ...
  public async handleOAuthCallback(code: string): Promise<boolean> {
    ...
    if (success) {
      // Re-initialize with OAuth access
  ...
  async toggleStudentStatus(studentId: string): Promise<boolean> {
    ...
    // TODO: Update in Google Sheets (requires finding and updating the specific row)
  ...
  async createTodo(userId: string, text: string): Promise<TodoRecord | null> {
    ...
    if (googleOAuthService.isAuthenticated()) {
      try {
        ...
        console.error('Failed to add todo to sheets:', error);
        return null;
      }
    }
  ...
  async deleteTodo(todoId: string): Promise<boolean> {
    ...
    // TODO: Delete from Google Sheets (requires finding and deleting the specific row)
  ...
  async clearCompletedTodos(userId: string): Promise<boolean> {
    ...
    // TODO: Update Google Sheets to remove completed todos
